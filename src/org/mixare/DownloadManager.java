...
public class DownloadManager implements Runnable {
	...
	public DownloadManager(MixContext ctx, Location l) {
		this.ctx = ctx;
		this.l = l;
	}
	...
	private DownloadResult processRequest(DownloadRequest request) {
		...
		try {
			String url1=request.url.split("&")[0];
			if(ctx.getHttpGETInputStream(url1)!=null){

				is = ctx.getHttpGETInputStream(url1);
