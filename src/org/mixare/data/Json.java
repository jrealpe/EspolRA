...
public class Json extends DataHandler {
	...
	//android10 added
	public void processAndroid10JSONObject(JSONObject jo, Location l) throws JSONException {
		...
		Double elevation;
		String webpage = "";
		//Documentacion aqui --> https://developers.google.com/maps/documentation/localsearch/jsondevguide?hl=es
		...
		if (jo.has("lat") && jo.has("lng") && jo.has("titleNoFormatting") && jo.has("streetAddress")) {
			...
			Log.d(MixView.TAG, "datos:"+title+"::"+address);	
			createMarker(title,
						 address,
						 phonenumber,
					     latitude,
	...
	public void load(JSONObject root, Location l) {
		JSONObject jo = null;
		...
		try {
			...
			if (dataArray != null) {
				...
				for (int i = 0; i < top; i++) {
					...
					processAndroid10JSONObject(jo,l);
